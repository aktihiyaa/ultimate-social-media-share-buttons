...
function SMSB_PAGE() {
    ...
    <h2 class="nav-tab-wrapper">
        <?php do_action('mozedia_smsb_settings_tab'); ?>
    </h2>
    <?php do_action('mozedia_settings_content'); ?>
    <?php
}
...
function mozedia_cos_sharing_global() {
    ...
    <div class="postbox" style="padding: 15px;">
        <input type="checkbox" name="mozedia-social-sharing-post-page-global" value="1" <?php checked(1, $value); ?> /> Enable (display share button on Post/Page)
    </div>
    <?php
}

function mozedia_cos_sharing_float_global() {
    $value = get_option('mozedia-social-sharing-float-global');
    ?>
    <div class="postbox" style="padding: 15px;">
        <input type="checkbox" name="mozedia-social-sharing-float-global" value="1" <?php checked(1, $value); ?> /> Enable (display share button in left side)
    </div>
    <?php
}

function mozedia_cos_sharing_title() {
    $value = get_option('mozedia-social-sharing-custom-label');
    ?>
    <div class="postbox" style="padding: 15px;">
        <input type="text" name="mozedia-social-sharing-custom-label" value="<?php echo esc_attr($value); ?>" /> (Like: Share, share this)
    </div>
    <?php
}

function mozedia_cos_sharing_post_page_options() {
    $facebook = get_option('mozedia-social-sharing-facebook');
    $twitter = get_option('mozedia-social-sharing-twitter');
    $linkedin = get_option('mozedia-social-sharing-linkedin');
    $email = get_option('mozedia-social-sharing-email');
    $pinterest = get_option('mozedia-social-sharing-pinterest');
    $whatsapp = get_option('mozedia-social-sharing-whatsapp');
    ?>
    <div class="postbox" style="padding: 30px;">
        <input type="checkbox" name="mozedia-social-sharing-facebook" value="1" <?php checked(1, $facebook); ?> /> Facebook
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-twitter" value="1" <?php checked(1, $twitter); ?> /> Twitter
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-linkedin" value="1" <?php checked(1, $linkedin); ?> /> Linkedin
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-email" value="1" <?php checked(1, $email); ?> /> Email
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-pinterest" value="1" <?php checked(1, $pinterest); ?> /> Pinterest
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-whatsapp" value="1" <?php checked(1, $whatsapp); ?> /> WhatsApp
    </div>
    <?php
}

function mozedia_cos_sharing_float_options() {
    $facebook = get_option('mozedia-social-sharing-float-facebook');
    $twitter = get_option('mozedia-social-sharing-float-twitter');
    $linkedin = get_option('mozedia-social-sharing-float-linkedin');
    $email = get_option('mozedia-social-sharing-float-email');
    $pinterest = get_option('mozedia-social-sharing-float-pinterest');
    $whatsapp = get_option('mozedia-social-sharing-float-whatsapp');
    ?>
    <div class="postbox" style="padding: 30px;">
        <input type="checkbox" name="mozedia-social-sharing-float-facebook" value="1" <?php checked(1, $facebook); ?> /> Facebook
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-float-twitter" value="1" <?php checked(1, $twitter); ?> /> Twitter
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-float-linkedin" value="1" <?php checked(1, $linkedin); ?> /> Linkedin
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-float-email" value="1" <?php checked(1, $email); ?> /> Email
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-float-pinterest" value="1" <?php checked(1, $pinterest); ?> /> Pinterest
        <br /><br /><input type="checkbox" name="mozedia-social-sharing-float-whatsapp" value="1" <?php checked(1, $whatsapp); ?> /> WhatsApp
    </div>
    <?php
}

function mozedia_cos_sharing_twitter() {
    $value = get_option('mozedia-social-sharing-twitter-name');
    ?>
    <div class="postbox" style="padding: 15px;">
        <input type="text" name="mozedia-social-sharing-twitter-name" value="<?php echo esc_attr($value); ?>" /> (without @)
    </div>
    <?php
}

function mozedia_cos_sharing_nofollow() {
    $value = get_option('mozedia-social-sharing-rel-nofollow');
    ?>
    <div class="postbox" style="padding: 15px;">
        <input type="checkbox" name="mozedia-social-sharing-rel-nofollow" value="1" <?php checked(1, $value); ?> /> add rel="nofollow" to all links
    </div>
    <?php
}

function mozedia_cos_top_padding() {
    $value = get_option('mozedia-social-sharing-top-padding');
    ?>
    <div class="postbox" style="padding: 15px;">
        <input type="text" style="width:50px;" name="mozedia-social-sharing-top-padding" value="<?php echo esc_attr($value); ?>" /> Set padding from top (Eg: 150-200)
    </div>
    <?php
}

function mozedia_cos_mobile_hide() {
    $value = get_option('mozedia-social-sharing-mobile-hide');
    ?>
    <div class="postbox" style="padding: 15px;">
        <input type="text" style="width:50px;" name="mozedia-social-sharing-mobile-hide" value="<?php echo esc_attr($value); ?>" /> (Recommended: 1200.)
    </div>
    <?php
}
...
function add_mozedia_social_sharing_buttons($content) {
    ...
    $rel_nofollow = (get_option("mozedia-social-sharing-rel-nofollow") == 1) ? 'rel="nofollow"' : '';
...
function mozedia_smsb_header() {
    ...
    .social-share .fa{font-family:icons;font-style:normal}
    .social-share .fa-facebook:before{content:"\f09a";}
    .social-share .fa-twitter:before{content:"\f099";}
    .social-share .fa-linkedin:before{content:"\f0e1";}
    .social-share .fa-pinterest:before{content:"\f231";}
    .social-share .fa-whatsapp:before{content:"\f232";}
    .social-share .fa-mail:before{content:"\e800";}
    .social-share .sharetitle{display:inline-block;float:left;font-size:18px;font-weight:600;border-right:1px solid #dfdfdf;padding:0 20px 0 5px;margin:5px 20px 0 0;color:#555;vertical-align:middle;text-align:center}
    .social-share.inline{display:block;margin:10px 0}
    .social-share .icon,.social-share .icon a{color:#fff;width:auto;height:auto;text-align:center;vertical-align:middle;overflow:hidden;white-space:nowrap;transition:.3s}
    .social-share .icon{display:inline-block;padding:0;min-width:auto;margin:2px;cursor:pointer}
    .social-share .icon a{display:block;font-size:15px;padding:7px 15px;border-radius:3px;line-height:1.5;}
    .social-share p:empty,.social-share ul>li:before{display:none}
    ...
    .social-share.flt .icon a,.social-share.flt .icon a:hover{line-height:1.7;padding:10px;text-align:center}
    .social-share.flt .icon:hover{width:auto;transition:.3s}
    .social-share .icon svg{width:14px;height:20px}
    .social-share .icon.flt-bar{padding:0 10px}
    .social-share .icon.flt-bar:hover{padding:0 20px}
    .social-share .flt-bar-items{display:none}
    .social-share .flt-bar,.social-share .flt-bar .flt-bar-button{display:inline}
    .social-share .flt-bar ul{margin:0;padding:0}
    .social-share .flt-bar #share-toggle,.social-share .flt-bar #share-toggle:checked+div .float-list,.social-share .flt-bar .float-list{opacity:0;transition:.3s}
    .social-share .flt-bar #share-toggle:checked+div .float-list{transform:translateX(-150%);-webkit-transform:translateX(-150%);-ms-transform:translateX(-150%)}
    .social-share .flt-bar .share-toggle:after{display:inline;content:"\f104"}
    .social-share .flt-bar #share-toggle:checked+div .share-toggle:after{content:"\f105"}
    .share-toggle,.social-share .flt-bar .float-list{opacity:1;transition:.3s}
    .social-share .flt-bar .share-toggle{color:#333;display:inline;line-height:1;margin:5px 13px;left:0;padding:3px 15px;cursor:pointer;font-family:icons;user-select:none;opacity:0;position:absolute;border-radius:100px}
    .social-share .flt-bar #share-toggle:checked+div .share-toggle,.social-share .flt-bar .share-toggle:hover{background:#777;color:#fff;opacity:1}
    .social-share:hover .flt-bar .share-toggle{opacity:1}
    .social-share .flt-bar #share-toggle:checked+div .share-toggle{left:0;margin:0}
    .social-share.flt .icon span{display:inline-block;width:0;height:0;overflow:hidden;transition:.3s;line-height:.8;position:absolute}
    .social-share .icon:hover span,.social-share.flt .icon:hover span{width:42px;height:auto;transition:.3s;position:relative}
    .social-share .icon span{font-size:12px;padding-left:5px;vertical-align:text-bottom;border-left:1px solid rgba(255,255,255,.1)}
    .social-share .icon.small{max-width:44px}
    .icon span{margin-left:5px;font-size:14px;text-decoration:none}
    .icon.facebook a{background:#4267b2}
    .icon.twitter a{background:#1da1f2}
    .icon.linkedin a{background:#0073b5}
    .icon.email a{background:#7d7d7d}
    .icon.pinterest a{background:#e60023}
    .icon.whatsapp a{background:#25d366}
    .socialicons{text-align:center}
    .icon.facebook a:hover{background:#365086}
    .icon.twitter a:hover{background:#1272ad}
    .icon.linkedin a:hover{background:#005983}
    .icon.email a:hover{background:#5d5d5d}
